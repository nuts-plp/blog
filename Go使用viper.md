...
**注意：**监控配置文件的第三方库[此库](https://github.com/fsnotify/fanotify)
...
```go
viper.SetConfigFile("./conf/") // 指定配置文件路径
...
**答：** 当本地有两个相同名称不同后缀的配置文件无论是否通过`viper.SetConfigType()`设置文件类型，仅读取第一个配置文件
...
在上面两个语句下搭配使用`viper.SetConfigType("yaml")`指定配置文件类型可以实现预期的效果吗？ 不能

```go
对于本地文件，解析时不需要指定文件格式，但对于远程配置系统获取的配置文件，需要viper.SetConfigType("yaml")指定配置文件类型，告诉系统以怎样的格式去解析文件.
```

示例      vipertest{main.go,go.mod,config.yaml}
...
func main() {
	...
	viper.Set("Host", "sdasda")
	fmt.Println(app.Host)
	fmt.Println(viper.Get("Host"))
	set := viper.IsSet("Host") //仅检测通过viper.Set存储的内容
	fmt.Println(set)
}
...
通过`viper.WatchConfig()监控文件，并通过Viper.OnConfigChange(func(e fsnotify.Event){})对改变做出处理`
